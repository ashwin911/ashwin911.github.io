(function($){'use strict';$(window).on('load',function(){$('.preloader').fadeOut(100);});$(window).on('load',function(){if(window.location.pathname!='/'){$('.navbar-logo').addClass('nav-bg nohome top');}});$(window).scroll(function(){if($(window).scrollTop()>100){$('.navigation').addClass('nav-bg');if(window.location.pathname!='/'){$('.navbar-logo').removeClass('top');}else{$('.navbar-logo').addClass('nav-bg');}}else{$('.navigation').removeClass('nav-bg');if(window.location.pathname=='/'){$('.navbar-logo').removeClass('nav-bg');}else{$('.navbar-logo').addClass('top');}}});$('[data-background]').each(function(){$(this).css({'background-image':'url('+$(this).data('background')+')'});});$('[data-color]').each(function(){$(this).css({'background-color':$(this).data('color')});});$('[data-progress]').each(function(){$(this).css({'bottom':$(this).data('progress')});});window.onload=function(){var parallaxBox=document.getElementById('parallax');var imageelement=document.getElementById('l2');if(imageelement==undefined)
return;var
...
$(document).ready(function(){$('#triggerGDPR').click(function(){$('#overlayGDPR').fadeIn(300);});$('#closeGDPR').click(function(){$('#overlayGDPR').fadeOut(300);});});})(jQuery);